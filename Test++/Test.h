...
template<typename T, typename U>
inline void test_assert_str_eq_(T&& t, U&& u)
{
	EXPECT_STREQ(t.c_str(), u.c_str());
	EXPECT_STREQ(u.c_str(), t.c_str());
}
#define TEST_ASSERT_EQ_STR(X1, X2) { test_assert_str_eq_(std::string(X1), std::string(X2)); }
...
#define TEST_ASSERT_NOT_NULL(X) TEST_ASSERT_NOT_EQ(X, nullptr)
