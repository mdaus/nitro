...
template<typename T> class AckMulticastSender
{
    ...
    int    mRetransmitPort;
public:
    AckMulticastSender(const std::string& mcastGroup,
		       int mcastPort,
		       int localAckPort, 
		       const std::vector<std::string>& subscribers,
		       int retransmitPort,
		       int loopback = 1) : mRetransmitPort(retransmitPort)
    {
	mSubscribers = subscribers;
	mMulticastSender = createSenderSocket(mcastGroup, 
					      mcastPort, 
					      loopback);
    ...
    Socket createSenderSocket(const std::string& mcastGroup,
			      int mcastPort, 
			      int loopback)
    {
	...
	for (int i = 0; i < mSubscribers.size(); i++)
	{
	    ...
	    //int rv = 1;
	    int rv = ::select(mAckChannel.getHandle()+1, &readers, NULL, NULL, &tv);
	    if (rv < 0)
	    {
...
int main(int argc, char** argv)
{
    ...
    {
	if (argc < 6)
	    die_printf("Usage: %s <multicast-group> <multicast-local-port> <our-ack-port> <retransmit-port> <subscriber1> ... <subscriberN>\n", argv[0]);
