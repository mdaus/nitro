...
void jumpThroughHoops(const linear::lite::Matrix<double>& m)
{
	...
	linear::lite::Vector<double> vec = 
	  VectorCreator().newConstantVector(4, 1);
	std::cout << altS * vec << std::endl;
	std::cout << vec << std::endl;

	linear::lite::Matrix<double> vecx = 
	  MatrixCreator().newConstantMatrix(4, 1, 1);
...
int main()
{
 	...
 	linear::lite::Matrix<double> myMx = MatrixCreator().newConstantMatrix(4, 4, 1);
 	initMx(myMx);
