...
void createSingleBandBuffer(std::vector<sys::byte>& buffer,
                            ...
                            const size_t segmentIdxToMakeEmpty)
{
   ...
   for (uint32_t segIdx = 0; segIdx < segments.size(); ++segIdx)
   {
      ...
      {
         segStart[segmentSizeInBytes/2] = static_cast<sys::byte>(0xff);
      }
   }
}

void createBuffers(std::vector<std::vector<sys::byte> >& buffers,
...
TEST_CASE(testBlankSegmentsValid)
{
   ...
   std::vector<std::vector<sys::byte> > buffers;
