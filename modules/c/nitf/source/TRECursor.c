        ...
        nitf_Error * error)
{
    ...
    strcpy_s(cursor.tag_str, NITF_TRECursor_tag_str_LEN, tre_cursor->tag_str);
                                                ...
                                                int looping, nitf_Error * error)
{
    ...
    strncpy_s(tag_str, TAG_BUF_LEN, descTag, sizeof(tag_str)-1);
    ...
    if (strchr(descTag, '['))
    {
        ...
        *(strchr(tag_str, '[')) = 0;
        ...
        {
            /* tack on the depth */
            strcat_s(tag_str, TAG_BUF_LEN, idx_str[index++]);
        }
    }
    else
    {
        ...
        {
            strcat_s(tag_str, TAG_BUF_LEN, idx_str[i]);
            pair = nitf_HashTable_find(
                    ((nitf_TREPrivateData*)tre->priv)->hash, tag_str);
        }
    }
                    ...
                    (tre_cursor->desc_ptr->data_type == NITF_BINARY))
            {
                ...
                if (tre_cursor->looping)
                {
                    ...
                    for (index = 0; index < tre_cursor->looping; index++)
                    {
                        ...
                        strcat_s(tre_cursor->tag_str, NITF_TRECursor_tag_str_LEN, entry);
                    }
                }
                                      ...
                                      int looping, nitf_Error* error)
{
    ...
    else
    {
        ...
        if (desc_ptr->label && strlen(desc_ptr->label) != 0)
        {
            ...
            strcpy_s(str, TAG_BUF_LEN, desc_ptr->label);
            op = str;
            while (isspace(*op))
                op++;
