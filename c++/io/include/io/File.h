...
public:
     ...
     */
    File(const sys::Path& path, int accessFlags = READ_ONLY, 
         int creationFlags = EXISTING) throw (sys::SystemException)
    {
        create(path.getPath(), accessFlags, creationFlags);
    }

    File(const sys::Path& parent, std::string name, int accessFlags = READ_ONLY,
         int creationFlags = EXISTING) throw (sys::SystemException)
    {
     ...
     */
    void readInto(char* buffer, sys::Size_T size) throw (sys::SystemException);
    ...
    void writeFrom(const char* buffer, 
                   sys::Size_T size) throw (sys::SystemException);
    ...
    {
        return seekTo(0, io::File::FROM_CURRENT);
    }
