...
#ifndef __MT_THREAD_IRIX_H__
#define __MT_THREAD_IRIX_H__

#if defined(__sgi)
#include <mt/SyncFactoryIrix.h>
#include <mt/ThreadInterface.h>
#include <string>
...
public:
    ...
    virtual void start()
    {
        ...
        if (!mt::SyncFactoryIrix().startThread(*this))
            throw mt::MultiThreadException("start()");
    }

    virtual void kill()
    {
        dbg_printf("ThreadIrix::kill()\n");
        if (!mt::SyncFactoryIrix().killThread(*this))
            throw mt::MultiThreadException("kill()");
    }
    ...
    static void __start(void *v)
    {
        ...
        mt::ThreadIrix *me = (mt::ThreadIrix *)v;
        me->target()->run();
