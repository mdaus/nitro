...
#  define TEST_ASSERT(X) if (!(X)) { die_printf("%s (%s,%s,%d): FAILED: Value should not be nullptr\n", testName.c_str(), __FILE__, SYS_FUNC, __LINE__); }
#  define TEST_ASSERT_NULL(X) if ((X) != nullptr) { die_printf("%s (%s,%s,%d): FAILED: Value should be nullptr\n", testName.c_str(), __FILE__, SYS_FUNC, __LINE__); }
...
#  define TEST_ASSERT(X) if (!(X)) { \
    fprintf(stderr, "%s (%s,%s,%d) : FAILED: Value should not be nullptr\n", testName, TEST_FILE, TEST_FUNC, TEST_LINE); \
    exit(EXIT_FAILURE); \
}
#  define TEST_ASSERT_NULL(X) if ((X) != nullptr) { \
    fprintf(stderr, "%s (%s,%s,%d) : FAILED: Value should be nullptr\n", testName, TEST_FILE, TEST_FUNC, TEST_LINE); \
    exit(EXIT_FAILURE); \
}
